...
async fn main() {
  let config = {
    ...
    CONFIG_CELL.get_or_init(|| async {
      const CONFIG_PATH: &str = "config.toml";
      if let Ok(mut file) = fs::File::open(CONFIG_PATH).await {
...
async fn serve_video(
  ...
  let video_path = config.video_path.join(video);
